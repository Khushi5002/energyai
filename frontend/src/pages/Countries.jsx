  ...
  return (
    <div>
      <header className="mb-6 sm:mb-8">
        <h1 className="font-display text-xl font-semibold tracking-tight sm:text-2xl">Supplier Countries</h1>
      ...
      <div className="rounded-xl overflow-x-auto" style={{ background: "var(--panel)", border: "1px solid var(--panel-border)" }}>
        <table className="w-full min-w-[620px] text-sm">
          <thead>
            <tr className="text-left text-xs text-zinc-500 font-mono-data border-b" style={{ borderColor: "var(--panel-border)" }}>
              ...
              <th className="p-3 sm:p-4">Final Score</th>
          ...
          <tbody>
            {countries.map((c, i) => (
              <tr key={i} className="border-b last:border-0" style={{ borderColor: "var(--panel-border)" }}>
                <td className="p-3 sm:p-4 font-medium">{COUNTRY_NAMES[c.iso] || c.iso} <span className="text-zinc-600 text-xs">({c.iso})</span></td>
                <td className="p-3 sm:p-4 font-mono-data text-zinc-400">{c.baseline}</td>
                <td className="p-3 sm:p-4 font-mono-data text-zinc-400">{c.live_factor}</td>
                <td className="p-3 sm:p-4 font-mono-data font-semibold" style={{ color: riskColor(c.score) }}>{c.score}</td>
